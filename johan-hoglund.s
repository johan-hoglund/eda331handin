...
# Use t6 to contain [k][k] address
move $t6,	$a0 # Set (k, k) to (0, 0), data register start
...
#		div.s $f0, $f3, $f0 # Store 1/A[k][k] for re-use later, since multiplication is cheaper than division
		...
		li $t1,	0 # Variable j
		jloop:
			mul $t4,	$t2, 	$a1 # k (row number) * (elements per row)
			addu $t4,	$t4,	$t1
		...
		iloop:
				...
				addu $t3,	$t3, $t1 # move to [i][j]
				sll $t3, $t3, 2 # *= 4
				addu $t3, $t3, $a0 # Add base addess

				mul $t5, $t2, $a1 # k * (elements per row)
				addu $t5, $t5, $t1 # += j
				sll $t5, $t5, 2
